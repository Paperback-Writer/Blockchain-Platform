...
<body>
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md-9">
                ...
                <!-- Update 按钮 + 状态提示（AJAX 版） -->
                <div id="update-panel" class="mt-2">
                    <button id="update-btn" class="btn btn-primary btn-sm">
                        Update images from remote
                    </button>
                    <span id="update-status" class="ml-2" style="display:none;">Updating...</span>
                </div>

                <script>
                (function(){
                // 取当前选择（这些模型变量在 Controller 已经放入 model）
                    var params = {
                    ...
                    btn.addEventListener('click', function(e){
                        ...
                        .then(function(){
                            // 同步完成后刷新页面（加时间戳规避缓存）
                            var url = '${contextPath}/blockchain.html'
                                + '?cryptocurrency=' + encodeURIComponent(params.cryptocurrency)
                                + '&metric='         + encodeURIComponent(params.metric)
                                + '&analysisType='   + encodeURIComponent(params.analysisType)
                                + '&_=' + Date.now();
                            ...
                            <c:forEach items="${images}" var="image" varStatus="status">
                                <div class="col-md-12 mb-4">
                                    ...
                                    <div class="blockchain-image-container">
                                        <!-- 添加版本参数 -->
                                        <img src="${fn:startsWith(image.path, '/') ? image.path : (contextPath.concat('/').concat(image.path))}?v=${now.time}" alt="${image.title}">
    ...
    <footer class="footer">
        <div class="container-fluid">
            <p class="mb-0">&copy; 2025 Zhongxing Du. zhongxing.du@uzh.ch</p>
        </div>
    </footer>
