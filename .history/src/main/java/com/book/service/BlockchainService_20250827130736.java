...
public class BlockchainService {
    ...
    public List<Image> getBlockchainImages(String cryptocurrency, String metric, String analysisType) {
        ...
        if ("cluster".equals(folderAnalysisType)) {
            // cluster 不区分币种，目录如：/static/blockchain-images/cluster/{metricFolder}/{fileName}
            metricFolder = CLUSTER_METRIC_FOLDER_MAP.getOrDefault(metric, metric.replace(" ", "_"));
            basePath = "/static/blockchain-images/cluster/" + metricFolder + "/";
        } else {
            // 普通模式，目录如：/static/blockchain-images/{crypto}/{metricFolder}/{analysisType}/{fileName}
            metricFolder = METRIC_FOLDER_MAP.getOrDefault(metric, metric.replace(" ", "_"));
            basePath = "/static/blockchain-images/" + cryptocurrency + "/" + metricFolder + "/" + folderAnalysisType + "/";
        }


        for (ChartInfo chartInfo : chartInfos) {
            String fileName = new File(chartInfo.getPath()).getName(); // 只用文件名
            String path = basePath + fileName;
            images.add(new Image(chartInfo.getTitle(), path));
        }
        
        // 第3步：如果没有找到预定义的图表，则扫描目录查找所有图片s
        // if (images.isEmpty()) {
